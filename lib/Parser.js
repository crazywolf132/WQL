...
class Parser extends _Lexer.default {
  ...
  parseDefault() {
    let comparitor = this.expectMany(_TokenType.TokenType.EQUALS, _TokenType.TokenType.GT, _TokenType.TokenType.LT, _TokenType.TokenType.NOT); // checking if the is a value, then a '?'... as this would
  ...
  parseReference() {
    ...
    const alias = this.eat(_TokenType.TokenType.AS) ? this.parseIdentifier() : null; // if (this.match(TokenType.NUMBER) || this.match(TokenType.IDENTIFIER)) {
